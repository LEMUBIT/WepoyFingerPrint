...
internal object Util {
    ...
    fun updateFingerprintImage(mainActivity: MainActivity, fi: FingerprintImage?) {
        var fpBmp: ByteArray? = null
       lateinit var bitmap: Bitmap
        if (fi == null || (fi.convert2Bmp()) == null || ( BitmapFactory.decodeByteArray(fpBmp, 0, fpBmp!!.size)) == null) {
            bitmap = BitmapFactory.decodeResource(mainActivity.resources, R.drawable.nofinger)
        }
        mainActivity.mHandler.sendMessage(mainActivity.mHandler.obtainMessage(MainActivity.MSG_UPDATE_IMAGE, bitmap))
    }

    fun closeDevice(enrolActivity: EnrolActivity, finish: Boolean) {
        object : Thread() {
            override fun run() {
                ...
                if (( enrolActivity.mScanner.powerOff()) != FingerprintScanner.RESULT_OK) {
    ...
    fun openDevice(enrolActivity: EnrolActivity) {
